...
check('pending one-shot timer survives file load', bpy.app.timers.is_registered(addon.tick))
addon.sync_mode('DARK', prefs)
...
check('disable removes animation and pending timers', not bpy.app.timers.is_registered(fade.frame) and not bpy.app.timers.is_registered(addon.tick))
